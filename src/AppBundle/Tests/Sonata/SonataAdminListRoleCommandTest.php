...
class SonataAdminListRoleCommandTest extends CommandTestCase
{
    public function testListing()
    {
        ...
        foreach (self::getAdminList() as $def) {
            list($title, $description, $performance, $employee) = $def;
            $this->assertContains($title, $output);
            $this->assertContains($description, $output);
            $this->assertContains($performance, $output);
            $this->assertContains($employee, $output);
        }
    }
}